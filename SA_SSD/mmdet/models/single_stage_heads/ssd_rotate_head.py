...
def gen_sample_grid(box, window_size=(4, 7), grid_offsets=(0, 0), spatial_scale=1.):
    ...
    x_half180 = torch.rot90(x[2:, :, :], 2, [0, 1])
    y_half180 = torch.rot90(y[2:, :, :], 2, [0, 1])
    x = torch.cat([x[:2, :, :].view(win//2, 1, N), x_half180.view(win//2, 1, N)], dim=1)
    y = torch.cat([y[:2, :, :].view(win//2, 1, N), y_half180.view(win//2, 1, N)], dim=1)
    return x, y
    # return x.view(win, 1, -1), y.view(win, 1, -1)
...
class PSWarpHead(nn.Module):
    ...
    def forward(self, x, guided_anchors, is_test=False):
        ...
        for i, ga in enumerate(guided_anchors):
            ...
            score = out.view(28, -1).mean(dim=0).view(-1)
    ...
    def get_rescore_bboxes(self, guided_anchors, cls_scores, img_metas, cfg):
        ...
        for i in range(len(img_metas)):
            ibbox_pred = guided_anchors[i]
            iscores = cls_scores[i]

            if iscores.numel == 0:
                det_bboxes.append(None)
                det_scores.append(None)

            ibbox_pred = ibbox_pred.view(-1, 7)
            iscores = torch.sigmoid(iscores).view(-1)
            select = iscores > cfg.score_thr

            bbox_pred = ibbox_pred[select, :]
            scores = iscores[select]
    ...
    def get_rescore_bboxes1(self, guided_anchors, cls_scores, img_metas, cfg):

        ibbox_pred = guided_anchors
        iscores = cls_scores

        if iscores.numel == 0:
            return torch.zeros((0, 7), dtype=torch.float32).cuda(), torch.zeros((0,), dtype=torch.float32).cuda()
