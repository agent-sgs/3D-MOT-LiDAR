...
from mmdet.datasets.kitti_utils import read_label, read_lidar, read_track_label, \
...
class KittiVideo(Dataset):
    def __init__(self, root, ann_file,
        ...
        self.with_mask = with_mask
        self.with_point = with_point
        self.img_prefix = osp.join(root, 'image_02')
        self.right_prefix = osp.join(root, 'image_3')
        self.lidar_prefix = osp.join(root, 'velodyne_reduced')
        self.calib_prefix = osp.join(root, 'calib')
        self.label_prefix = osp.join(root, 'label_02')

        with open(ann_file, 'r') as f:
            self.sample_ids = list(map(str, f.read().splitlines()))
    ...
    def prepare_train_img(self, idx):
        ...
        img = mmcv.imread(osp.join(self.img_prefix, s_id, '%s.png' % i_id))
        img, img_shape, pad_shape, scale_factor = self.img_transform(img, 1, False)

        objects = read_track_label(osp.join(self.label_prefix, '%s.txt' % s_id))
        calib = Calibration(osp.join(self.calib_prefix, '%s.txt' % s_id), from_video=True)

        objects = [object for object in objects if object.frame_id == int(i_id) and object.type not in ["DontCare"]]
        gt_bboxes = np.array([object.box3d for object in objects], dtype=np.float32)
        gt_types = [object.type for object in objects]
        ...
        if self.with_point:
            points = read_lidar(osp.join(self.lidar_prefix, s_id, '%s.bin' % i_id))
        ...
        if isinstance(self.generator, VoxelGenerator):
            ...
            if self.anchor_area_threshold >= 0 and self.anchors is not None:
                dense_voxel_map = sparse_sum_for_anchors_mask(coordinates, tuple(grid_size[::-1][1:]))
                dense_voxel_map = dense_voxel_map.cumsum(0)
                dense_voxel_map = dense_voxel_map.cumsum(1)
                anchors_area = fused_get_anchors_area(dense_voxel_map, self.anchors_bv, voxel_size, pc_range, grid_size)
                anchors_mask = anchors_area > self.anchor_area_threshold
        ...
        # skip the image if there is no valid gt bbox
        if len(gt_bboxes) == 0: return None

        # limit rad to [-pi, pi]
        gt_bboxes[:, 6] = limit_period(gt_bboxes[:, 6], offset=0.5, period=2 * np.pi)
