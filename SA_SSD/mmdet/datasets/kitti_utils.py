...
class Tracklet3d(object):
    ...
    def __init__(self, label_file_line):
        ...
        try:
            ...
            self.score = 1.
        self.box3d = np.array([data[13], data[14], data[15], data[11], data[12], data[10], data[16]]).astype(np.float32)
...
class Calibration(object):
    ...
    def read_calib_from_video(self, calib_filepath):
        ...
        data = {}
        data['P0'] = np.array([float(x) for x in lines[0][1:13]], dtype=np.float32)
        data['P1'] = np.array([float(x) for x in lines[1][1:13]], dtype=np.float32)
        data['P2'] = np.array([float(x) for x in lines[2][1:13]], dtype=np.float32)
        data['P3'] = np.array([float(x) for x in lines[3][1:13]], dtype=np.float32)

        data['R0_rect'] = np.array([float(x) for x in lines[4][1:10]], dtype=np.float32)
        data['Tr_velo_to_cam'] = np.array([float(x) for x in lines[5][1:13]], dtype=np.float32)
        data['Tr_imu_to_velo'] = np.array([float(x) for x in lines[6][1:13]], dtype=np.float32)
        return data
...
def cart2hom(pts_3d):
    ...
    pts_3d_hom = np.concatenate([pts_3d, np.ones(n + [1], dtype=np.float32)], axis=-1)
    return pts_3d_hom
...
if __name__ == '__main__':
    calib_path = '/opt/projects/data/kitti/track/training/calib/0000.txt'
    calib = Calibration(calib_path, from_video=True)

    points = np.array([[2, 3, 4]], dtype=np.float32)
    pt = project_rect_to_image(points, calib)
    print(pt)
