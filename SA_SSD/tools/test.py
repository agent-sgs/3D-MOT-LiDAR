...
def single_test(model, data_loader, saveto=None, class_names=['Car']):
    template = '{} ' + ' '.join(['{:.6f}' for _ in range(15)]) + '\n'
    ...
    for i, data in enumerate(data_loader):
        ...
        for _ in range(batch_size):
            prog_bar.update()
    print("\n sec_per_example: {:.5f}".format((time.time()-start_time) / len(data_loader)))
    return annos
...
def main():
    ...
    if args.gpus == 1:
        ...
        NotImplementedError
    # kitti evaluation
    gt_annos = kitti.get_sequence_label_annos(dataset.label_prefix, dataset.sample_ids)
