...
def _calculate_num_points_in_gt(data_path, infos, relative_path, remove_outside=True, num_features=4):
    for info in infos:
        if relative_path: v_path = str(pathlib.Path(data_path) / info["velodyne_path"])
        else: v_path = info["velodyne_path"]
        points_v = np.fromfile(v_path, dtype=np.float32, count=-1).reshape([-1, num_features])
        rect = info['calib/R0_rect']
        Trv2c = info['calib/Tr_velo_to_cam']
        P2 = info['calib/P2']
        if remove_outside: points_v = remove_outside_points(points_v, rect, Trv2c, P2, info["img_shape"])

        # points_v = points_v[points_v[:, 0] > 0]
        annos = info['annos']
        kitti.dicts_select(annos, annos['name'] != 'DontCare')
...
def _create_reduced_point_cloud(data_path,
    ...
    for info in prog_bar(kitti_infos):
        ...
        # then remove outside.
        if back: points_v[:, 0] = -points_v[:, 0]
        points_v = remove_outside_points(points_v, rect, Trv2c, P2, info["img_shape"])

        if save_path is None:
            save_dir = pathlib.Path(str(v_path.parent.parent) + '_reduced') / v_path.parent.stem
            save_dir.mkdir(parents=True, exist_ok=True)
            save_filename = save_dir / v_path.name
            if back: save_filename += "_back"
        else:
...
def create_groundtruth_database(data_path,
    ...
    for info in prog_bar(kitti_infos):
        ...
        if relative_path: velodyne_path = str(root_path / velodyne_path)
        num_features = 4
        ...
        P2 = info['calib/P2']
        Trv2c = info['calib/Tr_velo_to_cam']
        if not lidar_only: points = remove_outside_points(points, rect, Trv2c, P2, info["img_shape"])
        ...
        for i in range(num_obj):
            ...
            if names[i] in used_classes:
                if relative_path: db_path = str(database_save_path.stem + "/" + filename)
                else: db_path = str(filepath)
                db_info = {
                    "name": names[i],
                    "path": db_path,
                    "image_idx": image_idx,
...
if __name__ == '__main__':
    # create_kitti_info_file('/opt/projects/data/kitti/track/sa-ssd')
    # create_reduced_point_cloud('/opt/projects/data/kitti/track/sa-ssd')

    create_groundtruth_database(data_path='/opt/projects/data/kitti/track/sa-ssd', \
                                info_path='/opt/projects/data/kitti/track/sa-ssd/kitti_infos_trainval.pkl', \
                                db_info_save_path='/opt/projects/data/kitti/track/sa-ssd/kitti_dbinfos_trainval.pkl')