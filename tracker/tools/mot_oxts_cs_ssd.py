...
from lib.tracker3d_oxts_cs import Tracker3D
from lib.common_utils import oxts_kitti
from lib.ssd_utils import cs_utils
...
eval_dir = pathlib.Path('/home/sgs/rtmot/oxtsA') / 'SA-SSD' / ('Car_'+args.split+'_h'+str(args.high_score)+'_c2'+'_tc1.1') / 'data'
if os.path.exists(eval_dir): shutil.rmtree(eval_dir)
os.makedirs(eval_dir)
print('#'*50+'\n', str(eval_dir))

###  3D DET  ###
mot_detector = Runner(cfg_file='/opt/projects/RTMOT/SA_SSD/configs/car_trk_cfg.py',
                      checkpoint='/opt/projects/RTMOT/SA_SSD/saved_model_vehicle/checkpoint_epoch_80.pth')
...
for sdx in eval_sequences:
  ...
  for idx in tqdm(image_ids):
    ...
    tic = time.time()
    tracked_boxes, trk_boxes = mot_tracker.predict(meta)
    sum_times += time.time() - tic

    iboxes3d, iscores, boxes3d, scores, extra_boxes3d, extra_scores = \
      mot_detector.forward(image_idx, tracked_boxes)
    torch.cuda.synchronize()

    tic = time.time()
    trk_boxes = torch.from_numpy(trk_boxes).float().cuda()
    num_det, trk_ious, cand_boxes, cand_scores = \
      cs_utils.select_condidate(iboxes3d, iscores, boxes3d, scores, extra_boxes3d, extra_scores, trk_boxes)

    trk_ids, trk_bboxes, trk_scores = \
      mot_tracker.update(num_det, len(tracked_boxes), trk_ious, cand_boxes, cand_scores, meta)
