import numpy as np
from .motion_oxts_cs import KalmanBoxTracker as KalmanBoxTracker
...
class Tracker3D(object):
  ...
  def predict(self, meta):
    ...
    return tracked_boxes.astype(np.float32), trk_bboxes.astype(np.float32)

  def update(self, num_det, num_tracked, trk_ious, cand_boxes, cand_scores, meta):
    """
    ...
    for d, t in matched_tracked:
      self.tracked_trajectories[t].update(cand_boxes[d], cand_scores[d], meta['oxts'], d < num_det)

    for d, t in matched_unconf:
      if d < num_det:
        self.unconf_trajectories[t].update(cand_boxes[d], cand_scores[d], meta['oxts'], True)

    for d in unmatched_dets:
      if d < num_det:
        tracker = KalmanBoxTracker(cand_boxes[d], cand_scores[d], meta['calib'], meta['oxts'])
  ...
  def get_output(self, meta):
    ...
    for trajectory in self.tracked_trajectories:
      if trajectory.no_hit_streak >= self.max_ages or \
        trajectory.trk_streak >= self.max_ages:
