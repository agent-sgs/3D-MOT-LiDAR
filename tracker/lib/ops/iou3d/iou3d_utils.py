...
def boxes_bev_iou_cpu(boxes_a, boxes_b):
    ...
    iou3d_cuda.boxes_iou_bev_cpu(boxes_a_bev.contiguous(), boxes_b_bev.contiguous(), ans_iou)
...
def boxes_iou3d_cpu(boxes_a, boxes_b):
    ...
    # bev overlap
    overlaps_bev = boxes_a.new_zeros(torch.Size((boxes_a.shape[0], boxes_b.shape[0]))) # (N, M)
...
def boxes_iou3d_gpu(boxes_a, boxes_b):
    ...
    if boxes_a.shape[0] == 0 or boxes_b.shape[0] == 0:
        return torch.cuda.FloatTensor(torch.Size((boxes_a.shape[0], boxes_b.shape[0]))).zero_()
