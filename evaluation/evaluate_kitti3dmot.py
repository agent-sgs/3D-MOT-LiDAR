...
import mailpy
from box_util import boxoverlap, box2doverlap, box3doverlap
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="./evaluation", max_truncation = 0, min_height = 25, max_occlusion = 2,
                 mail=None, cls="car", eval_2dbev=True, eval_3diou=False, eval_2diou=False):
        ...
        # data and parameter
        self.gt_path           = os.path.join(gt_path, "labels/kitti/training/label_02")
        self.t_sha             = t_sha
            ...
            for line in f:
                ...
                if not loading_groundtruth:
                    if len(fields) == 17:
                        ...
                        t_data.score  = float(fields[17])      # tracklet score
                    else:
    ...
    def getThresholds(self, scores, num_gt, num_sample_pts=num_sample_pts):
        ...
        interval = 1.0 / num_sample_pts

        thresholds = []
        recalls = []
        for i in range(num_sample_pts):
            thresholds.append(i * interval)
            recalls.append(np.sum(scores >= i*interval) / float(num_gt))
            ...
            for f in range(len(seq_gt)):        # go through each frame
                ...
                for gg in g:
                    ...
                    for tt in t:
                        ...
                        elif self.eval_2dbev:
                            c = 1 - box2doverlap(gg, tt)
                        else:
                            assert False, 'error'
    ...
    def createSummary_details(self):
        ...
        summary += "evaluation: best results with single threshold".center(85,"=") + "\n"
    ...
    def createSummary_simple(self, threshold, recall):
        ...
        summary += ("evaluation with confidence threshold %f, recall %f" % (threshold, recall)).center(85,"=") + "\n"
        summary += ' MOTA   MOTP    MT     ML     IDS  FRAG    F1   Prec  Recall  FAR     TP    FP    FN\n'
...
class stat:
    ...
    def update(self, data, threshold_tmp):
        self.mota += data['mota']
        self.motp += data['motp']
        self.F1 += data['F1']
        # self.moda += data['moda']
        ...
        self.threshold_list.append(threshold_tmp)

    def output(self):
        self.amota = self.mota / num_sample_pts
        self.amotp = self.motp / num_sample_pts
    
    def print_summary(self):
        summary = ""
        
        summary += ("evaluation: average over confidence score").center(80,"=") + "\n"
        summary += ' AMOTA  AMOTP \n'

        summary += '{:.4f} {:.4f}\n'.format(self.amota, self.amotp)
        summary += "="*80
    ...
    def plot_over_score(self, data_list, title, y_name, save_path):
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot(np.array(self.threshold_list), np.array(data_list))
        ...
        if y_name in ['MOTA', 'F1']:
            ...
            plt.axvline(self.threshold_list[max_ind], ymax=data_list[max_ind], color='r')
            plt.plot(self.threshold_list[max_ind], data_list[max_ind], 'or', markersize=12)
            plt.text(self.threshold_list[max_ind]-0.2, data_list[max_ind]-0.1, '%.2f' % (data_list[max_ind] * 100), fontsize=40)
    ...
    def plot_over_recall(self, data_list, title, y_name, save_path):
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot(np.array(self.recall_list), np.array(data_list))
        # ax.set_title(title, fontsize=30)
        ...
        if y_name in ['MOTA', 'F1']:
            ...
            plt.text(self.recall_list[max_ind]-0.2, data_list[max_ind]-0.1, '%.2f' % (data_list[max_ind] * 100), fontsize=35)
        fig.savefig(save_path, transparent=True)
        plt.close()

    def plot_recall_over_score(self, save_path):
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot(self.threshold_list, self.recall_list)
        ax.set_ylabel("Recall", fontsize=35)
        ax.set_xlabel('Score', fontsize=35)
        ax.set_xlim(0.0, 1.0)
        plt.yticks(fontsize=30)
        plt.tight_layout()
        ax.set_ylim(0.0, 1.0)
    ...
    def plot_recall_and_score(self, data_list, save_path):
        fig = plt.figure()
        max_ind = np.argmax(np.array(data_list))

        ax = fig.add_subplot(212)
        ax.plot(np.array(self.threshold_list))
        ax.set_ylabel('Score', fontsize=35)
        ax.set_ylim(0.0, 1.0)
        plt.xticks(ticks=[0, 10, 20, 30, 40], fontsize=30)
        plt.yticks(ticks=[0.0, 0.5, 1.0], fontsize=30)
        plt.tight_layout()
        plt.axvline(max_ind, ymax=self.threshold_list[max_ind], color='r')
        plt.plot(max_ind, self.threshold_list[max_ind], 'or', markersize=12)
        plt.text(max_ind - 0.15, self.threshold_list[max_ind] - 0.15,
                 '%.2f' % (self.threshold_list[max_ind]), fontsize=30)

        ax = fig.add_subplot(211)
        ax.plot(np.array(self.recall_list))
        ax.set_ylabel('Recall', fontsize=35)
        ax.set_ylim(0.0, 1.0)
        plt.xticks(ticks=[0, 10, 20, 30, 40], fontsize=30)
        plt.yticks(ticks=[0.0, 0.5, 1.0], fontsize=30)
        plt.tight_layout()
        plt.axvline(max_ind, ymax=self.recall_list[max_ind], color='r')
        plt.plot(max_ind, self.recall_list[max_ind], 'or', markersize=12)
        plt.text(max_ind - 0.15, self.recall_list[max_ind] - 0.2,
                 '%.2f' % (self.recall_list[max_ind]), fontsize=35)
    ...
    def plot(self):
        ...
        self.plot_over_score(self.mota_list, 'MOTA - Score Curve', 'MOTA', os.path.join(save_dir, 'MOTA_score_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_score(self.motp_list, 'MOTP - Score Curve', 'MOTP', os.path.join(save_dir, 'MOTP_score_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_score(self.f1_list, 'F1 - Score Curve', 'F1', os.path.join(save_dir, 'F1_score_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_score(self.fp_list, 'False Positive - Score Curve', 'False Positive', os.path.join(save_dir, 'FP_score_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_score(self.fn_list, 'False Negative - Score Curve', 'False Negative', os.path.join(save_dir, 'FN_score_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_score(self.precision_list, 'Precision - Score Curve', 'Precision', os.path.join(save_dir, 'precision_score_curve_%s_%s.pdf' % (self.cls, self.suffix)))

        self.plot_recall_and_score(self.mota_list, os.path.join(save_dir, 'recall_score_curve%s_%s.pdf' % (self.cls, self.suffix)))
...
def evaluate(result_sha, mail, eval_2dbev, eval_3diou, eval_2diou):
    """
    ...
    if eval_2dbev:
        mail.msg("Processing Result for KITTI 2D BEV MOT Benchmark")
    elif eval_3diou:
    ...
    for c in ("car", "pedestrian"):
        e = trackingEvaluation(t_sha=result_sha, mail=mail,cls=c,
                               eval_2dbev=eval_2dbev,eval_3diou=eval_3diou,eval_2diou=eval_2diou)
        ...
        elif eval_2dbev: suffix = 'evalBEV'
        else: suffix = 'eval2D'
        ...
        for threshold_tmp, recall_tmp in zip(threshold_list, recall_list):
            ...
            stat_meter.update(data_tmp, threshold_tmp)
            mota_tmp = e.MOTA
...
if __name__ == "__main__":
    ...
    #
    if len(sys.argv)==3:
        eval_mot = (sys.argv[2]).upper()
        if eval_mot == '2D':
            eval_2dbev, eval_3diou, eval_2diou = False, False, True      # eval 2d
        elif eval_mot == '3D':
            eval_2dbev, eval_3diou, eval_2diou = False, True, False        # eval 3d
        elif eval_mot == 'BEV':
            eval_2dbev, eval_3diou, eval_2diou = True, False, False        # eval 3d
        else:
            print("Usage: python eval_kitti3dmot.py result_sha ?D(e.g. 2D or 3D)")
            sys.exit(1)
    else:
        eval_2dbev, eval_3diou, eval_2diou = True, False, False        # eval 2d BEV

    # evaluate results
    success = evaluate(result_sha,mail, eval_2dbev, eval_3diou, eval_2diou)
    print('-'*80)
    print("\n 3D trajectories %s have been successfully evaluated" % result_sha)
