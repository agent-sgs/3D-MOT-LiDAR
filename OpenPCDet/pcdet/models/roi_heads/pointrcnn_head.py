...
class PointRCNNHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, num_class=1):
        ...
        for k in range(len(xyz_mlps) - 1):
            ...
            shared_mlps.append(nn.ReLU(inplace=inplace))
        ...
        self.merge_down_layer = nn.Sequential(
            ...
            *[nn.BatchNorm2d(c_out), nn.ReLU(inplace=inplace)] if use_bn else [nn.ReLU(inplace=inplace)]
        )
    ...
    def forward(self, batch_dict):
        ...
        if 'extra_rois' in batch_dict.keys():
            assert batch_dict['batch_size'] == 1
            batch_dict['rois'] = batch_dict['rois'].squeeze(dim=0)
            num_rois = len(batch_dict['rois'])

            batch_dict['rois'] = torch.cat((batch_dict['rois'].squeeze(dim=0), batch_dict['extra_rois']), dim=0)
            batch_dict['rois'] = batch_dict['rois'].unsqueeze(dim=0)
        ...
        if not self.training:
            ...
            if 'extra_rois' in batch_dict.keys():
                batch_dict['extra_scores'] = torch.sigmoid(torch.reshape(batch_cls_preds[0, num_rois:], (-1,)))
                batch_dict['batch_cls_preds'] = batch_cls_preds[:num_rois]
                batch_dict['batch_box_preds'] = batch_box_preds[:num_rois]
            else:
